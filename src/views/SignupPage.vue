<template>
  <div class="signup-page animated-gradient-background container mt-4 mb-5" style="max-width: 500px;">
    <div class="card glass-card animate-fade-in-up interactive-card-lift interactive-card-shadow-primary">
          ...
          <ul v-if="formMessage.errors && formMessage.errors.length > 0" class="mb-0 mt-2">
            <li v-for="(err, index) in formMessage.errors" :key="index">
              ...
              {{ err.field ? `${err.field}: ` : '' }}{{ err.message }}
                           <!-- Error details for specific fields -->
...
/* Theme-adaptive glass card background and text */
.glass-card {
  border: none;
  box-shadow: 0 0.5rem 1rem rgba(0, 0, 0, 0.15);
  background: rgba(255,255,255,0.85); /* Always light in light mode */
  color: #212529;
...
/* Form inputs on glass (theme-adaptive, matches LoginPage.vue) */
.form-control {
